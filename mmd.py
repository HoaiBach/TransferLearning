...
from sklearn.metrics.pairwise import rbf_kernel as rbfDistance

_eps = 1e-8


# My own implementation of rbf_mmd - slower than using theano
def my_rbf_mmd2(X, Y, sigma):
    src = X
    tar = Y
    gamma = 1 / (2 * sigma**2)

    s2s = np.asarray(rbfDistance(src, src, gamma=gamma))
    t2t = np.asarray(rbfDistance(tar, tar, gamma=gamma))
    s2t = np.asarray(rbfDistance(src, tar, gamma=gamma))

    sSize = src.shape[0]
    tSize = tar.shape[0]

    KXX = np.sum(np.fliplr(s2s))/(sSize*sSize)
    KYY = np.sum(np.fliplr(t2t))/(tSize*tSize)
    KXY = np.sum(s2t)/(sSize*tSize)
